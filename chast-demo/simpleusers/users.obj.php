...
class SimpleUsers{
	...
	*/
	...
	public function createUser( $username, $password )
	{
		...
		$sql = "INSERT INTO users (`userId`, `uUsername`, `uPassword`, `uSalt`, `uActivity`, `uCreated`)VALUES (NULL, ?, SHA1(?), ?, NOW(), NOW())";
	...
	*/
	...
	public function getSingleUser( $userId = null )
	{
		...
		$sql = "SELECT userId, uUsername,uPassword, uActivity, uCreated,uAge,uEmail,uPhone FROM users WHERE userId=? LIMIT 1";
		if( !$this->stmt = $this->mysqli->prepare($sql) ){
			throw new Exception("MySQL Prepare statement failed: ".$this->mysqli->error);
		}
		...
		$this->stmt->bind_result($userId,$username, $userPwd, $activity, $created,$age,$email,$phone);
		$this->stmt->fetch();
		
		$user["userId"] = $userId;
		$user["uUsername"] = $username;
		$user["uPassword"] = $userPwd;
